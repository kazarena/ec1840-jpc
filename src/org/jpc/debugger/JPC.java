...
public class JPC extends ApplicationFrame implements ActionListener {
	...
	private JMenuItem processorFrame, physicalMemoryViewer, linearMemoryViewer, breakpoints, opcodeFrame, traceFrame,
			monitor, frequencies, codeBlockTreeFrame;
	...
	public void actionPerformed(ActionEvent evt) {
		...
		// {
		// OpcodeFrequencyFrame f = (OpcodeFrequencyFrame)
		// objects.getObject(OpcodeFrequencyFrame.class);
		// if (f != null)
		// bringToFront(f);
		// else
		// {
	...
	class Hz extends JLabel implements ActionListener {
		...
		public void actionPerformed(ActionEvent evt) {
			...
			setText("Decoded: (" + decoded + " x86 Instr) (" + optimised + " UBlocks) | Executed: (" + count
					+ " x86 Instr) (" + executed + " UBlocks) | " + fmt.format(mhz) + " MHz");
			lastCount = count;
			lastTime = now;
		}
	}
	...
	public void createBlankHardDisk() {
		try {
			...
			String sizeString = JOptionPane.showInputDialog(this, "Enter the size in MB for the disk",
