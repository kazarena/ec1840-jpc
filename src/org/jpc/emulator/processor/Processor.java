...
public class Processor implements HardwareComponent {
	...
	public static final int PROC_EXCEPTION_MF_09 = 0x09; // Coprocessor
															// Segment Overrun
	...
	public static final int PROC_EXCEPTION_MF_10 = 0x10; // Floating-Point
															// Error
	...
	private boolean interruptRaised = false;
	...
	public void setCR0(int value) {
		...
		if((changedBits & CR0_NOT_WRITETHROUGH) != 0)
			System.err.println("INFO: Unimplemented CR0 flags changed (" + Integer.toHexString(changedBits)
					+ "). Now is " + Integer.toHexString(value));
...
//			}
	...
	public final void handleSoftProtectedModeInterrupt(int vector) {
		...
		try {
			...
			// System.err.println("Triple-Fault: Unhandleable, machine will
			// halt!");
	...
	private final void followProtectedModeException(int vector, boolean hasErrorCode, int errorCode, boolean hardware,
			boolean software) {
		...
		case 0x0e: // Interrupt Handler: 32-bit Interrupt Gate
		{
			...
			case 0x1b: // Code, Execute/Read, Accessed
			{
				...
				if(targetSegment.getDPL() < currentPrivilegeLevel) {
					...
					if(hasErrorCode) {
						if((ss.getDefaultSizeFlag() && (esp < 16) && (esp > 0)) || !ss.getDefaultSizeFlag()
								&& ((esp & 0xffff) < 16) && ((esp & 0xffff) > 0))
							throw new ProcessorException(PROC_EXCEPTION_SS, 0, true);
					} else {
						if((ss.getDefaultSizeFlag() && (esp < 12) && (esp > 0)) || !ss.getDefaultSizeFlag()
	...
	private final void followVirtual8086ModeException(int vector, boolean hasErrorCode, int errorCode,
			boolean hardware, boolean software) {
		if(vector == PROC_EXCEPTION_PF)
			setCR2(linearMemory.getLastWalkedAddress());
				...
				if(targetSegment.getDPL() < currentPrivilegeLevel) {
					...
					if(hasErrorCode) {
						if((newStackSegment.getDefaultSizeFlag() && (esp < 40) && (esp > 0))
								|| !newStackSegment.getDefaultSizeFlag() && ((esp & 0xffff) < 40)
								&& ((esp & 0xffff) > 0))
							throw new ProcessorException(PROC_EXCEPTION_SS, 0, true);
					} else {
						if((newStackSegment.getDefaultSizeFlag() && (esp < 36) && (esp > 0))
								|| !newStackSegment.getDefaultSizeFlag() && ((esp & 0xffff) < 36)
								&& ((esp & 0xffff) > 0))
							throw new ProcessorException(PROC_EXCEPTION_SS, 0, true);
					}
	...
	public boolean initialised() {
		boolean result = ((physicalMemory != null) && (linearMemory != null) && (ioports != null)
				&& (interruptController != null) && (virtualClock != null));
		if(result && !started) {
			reset();
			started = true;
		}
	...
	public boolean updated() {
		boolean result = (physicalMemory.updated() && linearMemory.updated() && ioports.updated()
				&& interruptController.updated() && virtualClock.updated());
		return result;
	}
