...
//		drives[1].drive = driveType;
	...
	public void loadState(DataInput input) throws IOException {
		...
		ioportRegistered = false; // make sure the drives aren't reset when
									// driveset is passed in.
	}
	...
	public int[] ioPortsRequested() {
		return new int[] { IOPORT_BASE + 1, IOPORT_BASE + 2, IOPORT_BASE + 3, IOPORT_BASE + 4, IOPORT_BASE + 5,
				IOPORT_BASE + 7 };
	}
	...
	private int readData() {
		...
		if(++dataOffset == dataLength) {
			dataOffset = 0;
			/*
			 * Switch from transfer mode to status mode then from status mode to
			 * command mode
			 */
			if((dataState & STATE_STATE) == STATE_DATA) {
	...
	private int readDirection() {
		int retval = 0;
		if(((getDrive(0).driveFlags & FloppyDrive.REVALIDATE) != 0)
				|| ((getDrive(1).driveFlags & FloppyDrive.REVALIDATE) != 0))
			retval |= 0x80;
	...
	private void writeData(int data) {
		...
		if(dataOffset == 0) {
			...
			case 0x08:
				fifo[0] = (byte) 0xc0;
				fifo[1] = (byte) (0x20 | (drive.head << 2) | currentDrive);
				fifo[2] = (byte) drive.track;
				setFIFO(3, false);
				resetIRQ();
...
//				interruptStatus = 0xc0;
	...
	private void enqueue(FloppyDrive drive, int data) {
		...
		if(++dataOffset == dataLength) {
			...
			case 0x04:
				...
				fifo[0] = (byte) ((drive.readOnly << 6) | (drive.track == 0 ? 0x10 : 0x00) | (drive.head << 2)
						| currentDrive | 0x28);
				setFIFO(1, false);
				break;
			case 0x07:
			...
			case 0x4D:
				...
				/*
				 * Bochs BIOS is buggy and don't send format informations for
				 * each sector. So, pretend all's done right now...
				 */
				dataState &= ~STATE_FORMAT;
				stopTransfer((byte) 0x00, (byte) 0x00, (byte) 0x00);
				break;
	...
	private void startTransfer(int direction) {
		...
		if(dmaEnabled) {
			...
			if(((direction == DIRECTION_SCANE || direction == DIRECTION_SCANL || direction == DIRECTION_SCANH) && dmaMode == 0)
				...
				state |= CONTROL_BUSY;
				/*
				 * Now, we just have to wait for the DMA controller to recall
				 * us...
				 */
	...
	public int transferHandler(int nchan, int pos, int size) {
		...
		if((dataDirection == DIRECTION_SCANE) || (dataDirection == DIRECTION_SCANL)
				|| (dataDirection == DIRECTION_SCANH))
			status2 = 0x04;
		...
		for(startOffset = dataOffset; dataOffset < size;) {
			...
			case DIRECTION_READ:
				/* READ commands */
				dma.writeMemory(nchan, fifo, relativeOffset, dataOffset, length);
				break;
			case DIRECTION_WRITE:
				/* WRITE commands */
				dma.readMemory(nchan, fifo, relativeOffset, dataOffset, length);
				...
				/* SCAN commands */
			{
				...
				dma.readMemory(nchan, tempBuffer, 0, dataOffset, length);
			...
			if(relativeOffset == 0) {
				/* Seek to next sector */
				/*
				 * XXX: drive.sect >= drive.last_sect should be an error in fact
				 */
				if((drive.sector >= drive.lastSector) || (drive.sector == eot)) {
					drive.sector = 1;
