...
public class Keyboard extends AbstractHardwareComponent implements UserInputDevice, IOPortCapable {
	...
	private synchronized void writeCommand(byte data) {
		...
		case (byte) 0xff:
			...
			break;
		default:
			System.err.println("Unsupported Keyboard Command " + Integer.toHexString(0xff & data));
			break;
		}
	}
	...
	private synchronized void updateIRQ() {
		...
		if(queue.length != 0) {
			...
			if(0 != queue.getAux()) {
				...
				if((0 != (mode & KBD_MODE_KBD_INT)) && (0 == (mode & KBD_MODE_DISABLE_KBD)))
					irq1Level = 1;
			}
		}
	...
	public synchronized void putMouseEvent(int dx, int dy, int dz, int buttons) {
		...
		if((0 == (mouseStatus & MOUSE_STATUS_REMOTE)) && (queue.length < (KBD_QUEUE_SIZE - 16))) {
			for(;;) {
				/*
				 * if not remote, send event. Multiple events are sent if too
				 * big deltas
				 */
				mouseSendPacket();
	...
	public boolean initialised() {
		return ioportRegistered && (irqDevice != null) && (cpu != null) && (physicalAddressSpace != null)
				&& (linearAddressSpace != null);
	}

	public boolean updated() {
		return ioportRegistered && irqDevice.updated() && cpu.updated() && physicalAddressSpace.updated()
