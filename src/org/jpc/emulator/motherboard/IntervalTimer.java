...
public class IntervalTimer implements IOPortCapable, HardwareComponent {
	...
	static final long scale64(long input, int multiply, int divide) {
		// return
		// (BigInteger.valueOf(input).multiply(BigInteger.valueOf(multiply)).divide(BigInteger.valueOf(divide))).longValue();
	...
	class TimerChannel implements HardwareComponent {
		...
		private void latchStatus() {
			if(!statusLatched) {
				...
				status = ((getOut(IntervalTimer.this.getTimingSource().getTime()) << 7) | (nullCount ? 0x40 : 0x00)
						| (rwMode << 4) | (mode << 1) | bcd);
				statusLatched = true;
			}
		}
		...
		public void write(int data) {
			...
			case RW_STATE_MSB:
				...
				break;
			case RW_STATE_WORD:
				// null count setting is delayed until after second byte is
				// written
				inputLatch = data;
		...
		private int getCount() {
			long now = scale64(IntervalTimer.this.getTimingSource().getTime() - countStartTime, PIT_FREQ,
					(int) IntervalTimer.this.getTimingSource().getTickRate());
		...
		private int getOut(long currentTime) {
			long now = scale64(currentTime - countStartTime, PIT_FREQ, (int) IntervalTimer.this.getTimingSource()
					.getTickRate());
			switch(mode) {
			default:
			case MODE_INTERRUPT_ON_TERMINAL_COUNT:
		...
		private long getNextTransitionTime(long currentTime) {
			...
			nextTime = countStartTime
					+ scale64(nextTime, (int) IntervalTimer.this.getTimingSource().getTickRate(), PIT_FREQ);
