...
public class DualInterruptController implements InterruptController {
	...
	private int pollRead(int address)
	{
	    int ret = this.getIRQ();
	    if (ret < 0) {
		DualInterruptController.this.updateIRQ();
		return 0x07;
	    }
	    
	    if (0 != (address >>> 7)) {
	    	DualInterruptController.this.masterPollCode();
	    }
	...
	{
	    if (DualInterruptController.this.master == this)
		return true;
	    else
		return false;
	}
	...
	{
	    if (isMaster()) {
		return (DualInterruptController.this).toString() + ": [Master Element]";
	    } else {
		return (DualInterruptController.this).toString() + ": [Slave  Element]";
	    }
	}
    }
